...
public class SimpleDummyAction extends CloupiaPageAction {
	...
	private static final String formId = "foo.simple.dummy.form";
	private static final String ACTION_ID = "foo.simple.dummy.action";
	...
	public int validatePageData(Page page, ReportContext context, WizardSession session) throws Exception {
		...
		logger.info("foo foo foo " + form.getName() + " : " + form.getValue());
		int number = form.getNumber();
		logger.info("foo foo foo number = " + number);
		boolean boolValue = form.getBoolType();
		logger.info("foo foo foo bool = " + boolValue);
		String password = form.getPassword();
		logger.info("foo foo foo password = " + password);
		long dateLong = form.getDateLong();
		logger.info("foo foo foo date long = " + dateLong);
		long dateTime = form.getDateTime();
		logger.info("foo foo foo date time = " + dateTime);
		String listValue = form.getListValue();
		logger.info("foo foo foo list value = " + listValue);
		...
		if (form.getUploadFileName() != null && !form.getUploadFileName().equals("")) {
			...
			try {
				...
				logger.info("foo foo foo fileData.length = " + fileData.length);
		...
		for (String selected:selectedTableValues) {
			logger.info("foo foo foo selected tabular value = " + selected);
		}
